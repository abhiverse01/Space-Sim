...
class HeavenlyBody:
    ...
    def update_position(self, bodies, screen):
        total_fx = total_fy = 0
        for body in bodies:
            if self == body:
                continue
        ...
        orbit_x = self.x * SCALE + screen.get_width() // 2
        orbit_y = self.y * SCALE + screen.get_height() // 2
...
def main():
    ...
    while run:
        ...
        for body in bodies:
            body.update_position(bodies, screen)  # Pass the screen as an argument
            body.draw(screen, show_orbit)

        # Draw labels at the top-right corner
        label_y_offset = 10
        for body in bodies:
            label = font.render(f"{body.name}: {body.color}", True, body.color)
